import { Button, DatePicker, Form, Image, InputNumber } from 'antd';
...
import { getListRentedRoomByID } from '../../../store/managerRentedRoom.services/thunkAction';
...
import {
    bookingRoom1,
    getBookingRoom,
} from '../../../store/managerBookingRoom.services/thunkAction';
import {
    managerAccountAction,
    managerAccountReducer,
} from '../../../store/managerAccount.services/slice';
const Detail = () => {
    const wifi = (
        <svg
            viewBox="0 0 32 32"
    ...
    const { rentedRoomByID } = useSelector((state: RootState) => state.managerRentedRoom);
    const { AccountUser } = useSelector((state: RootState) => state.managerAccount);
    const [datess, setDates] = useState<{ arrivalDay: string; dayOut: string }>({
        arrivalDay: '',
        dayOut: '',
    });
    const AppDispatch = useAppDispatch();
    useEffect(() => {
        AppDispatch(getListRentedRoomByID(Number(param.id)));
        AppDispatch(getListLocationRoomByID(Number(param.loaction)));
        AppDispatch(managerAccountAction.getUser({}));
        AppDispatch(getBookingRoom());
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [datess]);
    // bookRoom
    const [bookRoom, setBookRoom] = useState<bookingRoom>({
        roomCode: 0,
        arrivalDay: '',
        dayOut: '',
        theNumberGuests: 1,
    ...
    const handleminusNumber = () => {
        if (bookRoom.theNumberGuests === 1) {
            return;
        }
        setBookRoom({ ...bookRoom, theNumberGuests: bookRoom.theNumberGuests - 1 });
    };
    const handlePlusNumber = (quantilyGuest: number) => {
        if (quantilyGuest === bookRoom.theNumberGuests) {
            return;
        }
        setBookRoom({ ...bookRoom, theNumberGuests: bookRoom.theNumberGuests + 1 });
    };
        ...
        return listLocationByID?.map((item, i) => {
            return (
                <>
                    <span>
                        <EnvironmentOutlined
                            style={{ display: 'flex', alignItems: 'center', color: '#FF385C' }}
                        />{' '}
                        {item.nameLocation},{' '}
                    </span>
    ...
    const renderConvenient = (icon: any, text: any, istext: any) => {
        if (istext) {
            return (
                <div className="tools__room">
                    <div className="icon__tools__room">{icon}</div>
                    <div>{text}</div>
                </div>
            );
        } else {
            return (
                <div className="tools__room">
                    <div className="icon__tools__room">{icon}</div>
                    <div className="text__tools__room">
    ...
    const onRangeChange = (dates: null | (Dayjs | null)[], dateStrings: string[]) => {
        if (dates) {
            ...
            setDates({ arrivalDay: dateStrings[0], dayOut: dateStrings[1] });
            if (startDate && endDate) {
                const numberOfDays = Math.abs(endDate.diff(startDate, 'day')) + 1;
                setCountDate(numberOfDays);
                AppDispatch(getBookingRoom());
            }
        }
    };
    const handleTotalPrice = (price: number) => {
        const sum = price * Number(countDate);

        return (
            <>
                {(Number(sum) * 22100).toLocaleString('vi', { style: 'currency', currency: 'VND' })}
            </>
        );
    };
    // checkBooking
    ...
    const [conflictingBooking, setConflictingBooking] = useState<bookingRoom | null>(null);
    const handleSubmitBookRoom = async (roomCode: number) => {
        if (!localStorage.getItem('user')) {
            toast.error('Bạn cần đăng nhập để có thể đặt phòng 😊');
            return;
        }
        if (datess.arrivalDay === '') {
            toast.error('Bạn cần chọn ngày đặt phòng 🥺');
            return;
        }

        

        const conflictingBooking = listBooking?.find(
            (booking) =>
                (datess.arrivalDay <= booking.dayOut && datess.dayOut >= booking.arrivalDay) ||
                (datess.arrivalDay >= booking.dayOut && datess.dayOut <= booking.arrivalDay),
        );
        if (conflictingBooking) {
            setConflictingBooking(conflictingBooking);
            toast.error(
                `Xin lỗi, đã có người khác đặt phòng từ ngày ${conflictingBooking.arrivalDay} đến ngày ${conflictingBooking.dayOut}.`,
            );
        } else {
            const bookRoom1: bookingRoom = {
                roomCode: roomCode,
                arrivalDay: datess.arrivalDay,
                dayOut: datess.dayOut,
                theNumberGuests: bookRoom.theNumberGuests,
                userCode: Number(AccountUser?.code),
            };
            AppDispatch(bookingRoom1(bookRoom1));
            AppDispatch(getBookingRoom());
    ...
    const handleListItem = () => {
        return rentedRoomByID?.map((item, i) => {
            const plainText = htmlToText(item.discription);
            return (
                <>
                    <h1>{item.roomName}</h1>
                    <div className="feeback__location">
                        <span>
                            <span className="icon__start">
                                <StarOutlined style={{ color: '#d9d42d' }} /> 4 -{' '}
                            </span>{' '}
                            <span style={{ borderBottom: '1px solid #c4c1c1' }}> 2 đánh giá</span> .{' '}
                        </span>
                        {handleLocation()}
                    </div>
                    <div className="image__detail__room">
                        <Image src={item.image} alt="" width="100%" />
                    </div>
                    <div className="content__detail__room">
                        <div className="content__detail__left">
                            <div className="box__content__title__left">
                                <div className="box__content__title__1">
                                    <h3>{item.roomName}</h3>
                                    <p style={{ color: '#A1A1A1', fontSize: '16px' }}>
                                        <span>{item.guest} Khách</span> -
                                        <span> {item.bedRoom} Phòng ngủ </span> -
                                        <span> {item.bed} Giường</span> -
                                        <span> {item.bathroom} Phòng tắm</span>
                                    </p>
                                </div>
                                <div className="box__content__title__2">
                                    <HomeOutlined className="icon__home" />
                                </div>
                            </div>
                            <div className="box__benefit__title__left">
                                <div className="box__benefit__title__left__1">
                                    <div>
                                        <svg
                                            viewBox="0 0 32 32"
                                            xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    <div className="box__benefit__content">
                                        <span className="box__benefit__title__left__Title">
                                            Trải nghiệm nhận phòng tuyệt vời
                                        </span>
                                        <span className="box__benefit__title__left__Content">
                                ...
                                <div style={{ marginTop: '10px' }}>
                                    <span style={{ fontWeight: '500' }}>{plainText}</span>
                                </div>
                            </div>
                            <h3>Nơi này có những gì cho bạn</h3>
                            <div className="tools__in__the__room">
                                {renderConvenient(wifi, 'Wifi', item?.wifi)}
                                {renderConvenient(pool, 'Hồ Bơi', item?.pool)}
                                {renderConvenient(iron, 'Bàn Là', item?.iron)}
                                {renderConvenient(
                                    parking,
                                    'Chỗ đỗ xe miễn phí tại nơi ở',
                                    item?.parking,
                                )}
                                {renderConvenient(kitchen, 'Bếp', item?.kitchen)}
                                {renderConvenient(airCon, 'Điều hòa ', item?.airConditioner)}
                                {renderConvenient(tv, 'TV', item?.tivi)}
                            </div>
                        </div>
                        <div className="content__detail__right">
                            <div className="box__buynow__room">
                                <div className="box__buynow__title">
                                    <div className="box__buynow__title__price">
                                        <p>
                                    ...
                                    <div className="box__buynow__title__vote">
                                        <span style={{ display: 'flex', alignItems: 'center' }}>
                                            <StarOutlined
                                                style={{ color: '#d9d42d', paddingRight: '5px' }}
                                            />
                                            4 -{' '}
                                            <span
                                                style={{
                                ...
                                <div className="date__book__room">
                                    <RangePicker format="DD-MM-YYYY" onChange={onRangeChange} />
                                </div>
                                <div>
                                    <div className="box__guets">
                                        <div className="box__guets__1">
                                            <span
                                                style={{
                                                    display: 'block',
                                                    fontSize: '12px',
                                            ...
                                            <span style={{ display: 'block', opacity: '0.8' }}>
                                                {bookRoom.theNumberGuests} Khách
                                            </span>
                                        </div>
                                        <div className="">
                                            <DownOutlined />
                                    ...
                                    <div className="box__count__people">
                                        <div className="box__count__people__1">
                                            <div className="box__count__people__2">
                                                <span
                                                    style={{
                                                        display: 'block',
                                                        fontSize: '16px',
                                            ...
                                            >
                                                ...
                                                >
                                                    {bookRoom.theNumberGuests}
                                                </span>
                                                <button
                                                    className=""
                                                    onClick={() => handlePlusNumber(item.guest)}
                                                >
                                                    +
                                                </button>
                                            </div>
                                    ...
                                    <div className="price__one__night">
                                        <div className="price__one__night__1">
                                            <span>
                                                {(Number(item.price) * 22100).toLocaleString('vi', {
                                                    style: 'currency',
                                                    currency: 'VND',
                                            ...
                                            <span>{handleTotalPrice(item.price)}</span>
                                        </div>
                                        <div className="price__one__night__1">
                                        ...
                                        <div className="price__one__night__1">
                                            <div className="form__group">
                                                <input type="radio" name="rdiname" id="" />
                                ...
                                <button
                                    className="btn__buy__now__room"
                                    onClick={() => {
                                        handleSubmitBookRoom(item.codeRoom);
                                    }}
                                >
                                    Đặt Phòng
                                </button>
    ...
    return (
        <div className="detail__room">
            {handleListItem()}
            <div className="">
                <h3 style={{ fontWeight: '500', marginBottom: '20px' }}>Bình Luận</h3>
                {/* <div className="">
          {
                ...
                <div className="" key={index}>
                  <div className="">
                    ...
                    <div className="">
                      ...
                      <span style={{ display: "block" }} className="">{item.ngayBinhLuan}</span>
                    </div>
                  </div>
                  <div className="">
                ...
                <div className="" style={{ marginBottom: '120px' }}>
                    <Form labelCol={{ span: 4 }} wrapperCol={{ span: 14 }} layout="horizontal">
                        ...
                        >
                            <InputNumber
                                size="large"
                                prefix={<StarOutlined style={{ fontSize: '17px' }} />}
                                min={1}
                                max={5}
                                placeholder="Số sao"
                        ...
                        <Form.Item>
                            <Button
                                style={{ backgroundColor: 'none !importion' }}
                                type="primary"
                                htmlType="submit"
                            >
                                Bình Luận
