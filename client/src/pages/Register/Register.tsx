...
import { accountTypes } from '../../services/managerAccout.services'
import { NavLink, useNavigate } from 'react-router-dom'
import { useAppDispatch } from '../../store'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faPhone, faUser } from '@fortawesome/free-solid-svg-icons'
import logo from "../../assets/logo/long-logo.png";
import { registerAirBnb } from '../../store/managerAccount.services/thunkAction'
...
const Register = () => {
  const {
    ...
    register,
    formState: { errors },
  } = useForm<accountTypes>({ mode: "onChange" })
    ...
    <div className='bg-gray-300 h-screen grid content-center'>
      <div className='flex justify-center  '>
        <form className="form_main" onSubmit={handleSubmit(async (value) => {
          await appDispatch(registerAirBnb(value))
          navigate("/login")
        })}>
          <div style={{ width: "100%", left: 0 }} ><NavLink to='/'><img style={{ width: "30%", left: 0 }} src={logo} alt="logo" className="navbar-logo" /></NavLink></div>
          ...
          <div className="inputContainer">
            ...
            <input placeholder="Name user" className="inputField" type="text"
              {...register("name", {
                required: "Vui lòng nhập tên tài khoản !",
              })} />
          </div>
          {errors && errors.name && <p className="text-[13px] text-red-500">{errors.name.message}</p>}
          <div className="inputContainer">
            <FontAwesomeIcon className="inputIcon" icon={faPhone} />
            <input placeholder="phone" className="inputField" type="text"
              {...register("phone", {
          ...
          {errors && errors.phone && <p className="text-[13px] text-red-500">{errors.phone.message}</p>}
          <button id="button">Sign Up</button>
          <div className="signupContainer">
            <div className="mt-12 text-sm font-display font-semibold text-black text-center">
              You have a account ? <span className="cursor-pointer sign__in" onClick={() => {
                navigate("/login")
              }}>Sign in</span>
            </div>
          </div>
