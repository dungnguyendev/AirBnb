...
import { loginAirBnb } from '../../store/managerAccount.services/thunkAction'
import { useSelector } from 'react-redux'
const Login = () => {
  const navigate = useNavigate()
  const { shouldRedirect } = useSelector((state: RootState) => state.managerAccount)
  useEffect(() => {
    ...
    <div className='bg-gray-300 h-screen grid content-center'>
      <div className='flex justify-center  '>
        <form className="form_main" onSubmit={handleSubmit(async (value) => {
          await appDispatch(loginAirBnb(value))
        })} >
          ...
          <div className="signupContainer">
            <div className="mt-12 text-sm font-display font-semibold text-black text-center">
              Don't have an account ? <span className="sign__up cursor-pointer" onClick={() => {
                navigate("/register")
              }}>Sign up</span>
            </div>
          </div>
